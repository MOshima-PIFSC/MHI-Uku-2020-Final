...
INP <- read.delim("C:\\Users\\Marc.Nadon\\Documents\\Work docs\\01_Projects\\002_Uku assessment\\0_R_Uku\\SS3\\025_Base\\Report.SSO", 
                  fill=FALSE, stringsAsFactors = FALSE, header=FALSE)

SSB <- matrix(   unlist(strsplit(INP[642:714,],split=" ")),ncol=4,nrow=73,byrow=T    )
SSB <- as.data.frame( SSB[,-1], stringsAsFactors=F )
colnames(SSB) <- c("Year","SSB","SD")
SSB[,2] <- as.numeric(SSB[,2])

REC <- matrix(   unlist(strsplit(INP[715:787,],split=" ")),ncol=4,nrow=73,byrow=T    )
...
ggsave(filename="SR relation.png",plot=SR_plot,width=10,height=6,units="cm",dpi=300)
